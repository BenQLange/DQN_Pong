...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(dtype=tf.uint8, shape=[None, img_height, img_width, nchannels*config.state_history])
        self.a = tf.placeholder(dtype=tf.int32, shape=[None])
        self.r = tf.placeholder(dtype=tf.float32, shape=[None])
        self.sp = tf.placeholder(dtype=tf.uint8, shape=[None, img_height, img_width, nchannels*config.state_history])
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        #i think i get it  #dimensions!!
        with tf.variable_scope(scope, reuse = reuse):
            out = tf.layers.flatten(state)
            out = tf.layers.dense(out, num_actions, use_bias = True)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        var_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,q_scope)
        var_target_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,target_q_scope)
        self.update_target_op = tf.group(*[tf.assign(var_target_q[i],var_q[i]) for i in range(len(var_target_q))])

        #is it enough??? idk how to verify??!?!?!
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_loss_op(self, q, target_q):
        ...
        Q_samp= tf.add(self.r, tf.multiply(1.0-tf.cast(self.done_mask,dtype=tf.float32),tf.multiply(config.gamma,tf.reduce_max(target_q,axis=1))))
        Q_action_taken = tf.one_hot(self.a,num_actions)
        Q_s_a = tf.reduce_sum(tf.multiply(Q_action_taken, q),1)
        #Q_s_a = tf.tensordot(Q_action_taken, tf.transpose(q),axes = 1)
    ...
    def add_optimizer_op(self, scope):
        ...
        #print(scope)
        var_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope)
        grads_and_vars = optimizer.compute_gradients(self.loss, var_q)
        if self.config.grad_clip == True:
            capped_grads_and_vars = [(tf.clip_by_norm(gv[0],self.config.clip_val),gv[1]) for gv in grads_and_vars]

        self.train_op = optimizer.apply_gradients(capped_grads_and_vars)
        self.grad_norm = tf.global_norm(capped_grads_and_vars)
