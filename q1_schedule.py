...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        if t <= self.nsteps:
            self.epsilon = self.eps_begin + (t-0)*(self.eps_end-self.eps_begin)/(self.nsteps-0)
        else:
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        return np.random.choice([random_action, best_action], 1, p=[self.epsilon, 1-self.epsilon])[0]
